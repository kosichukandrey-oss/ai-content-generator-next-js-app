...
const translations = {
  ru: { lang: "ru", title: "ContentAI Generator — контент, который работает", ai: "Ваш AI-копирайтер", badge: "Тексты для бизнеса и соцсетей за минуты", hero: ["Идея есть.", "Слова найдутся."], intro: "Выберите формат, расскажите о своей задаче — ContentAI подготовит контент, который хочется дочитать.", create: "Создаём контент", steps: "Выберите формат и опишите задачу", language: "Язык результата", format: "Выберите формат", topic: "Тема или продукт", topicPlaceholder: "Например: новая коллекция десертов", details: "Описание задачи", optional: "необязательно", detailsPlaceholder: "Расскажите о целевой аудитории, главной выгоде или желаемом призыве к действию", generate: "Сгенерировать контент", generating: "AI пишет текст…", result: "Ваш результат", resultHint: "Готовый текст появится здесь", empty: "Здесь будет ваш текст", emptyHint: "Заполните форму слева и нажмите «Сгенерировать контент».", copy: "Копировать текст", copied: "✓ Текст скопирован", ideas: "Не знаете, с чего начать?", validation: "Введите тему или название продукта — без неё AI не сможет начать.", fallback: "Что-то пошло не так. Попробуйте ещё раз.", types: [["Instagram пост", "публикация с призывом"], ["Instagram Stories", "серия вовлекающих экранов"], ["Telegram пост", "живой текст для канала"], ["Рекламное объявление", "три коротких варианта"], ["Описание товара", "выгоды и характеристики"], ["Идея контент-плана", "план публикаций на 7 дней"]], examples: ["новая коллекция десертов", "акция на кроссовки"] },
  uk: { lang: "uk", title: "ContentAI Generator — контент, що працює", ai: "Ваш AI-копірайтер", badge: "Тексти для бізнесу й соцмереж за хвилини", hero: ["Є ідея.", "Знайдуться слова."], intro: "Оберіть формат, розкажіть про своє завдання — ContentAI підготує контент, який хочеться дочитати.", create: "Створюємо контент", steps: "Оберіть формат і опишіть завдання", language: "Мова результату", format: "Оберіть формат", topic: "Тема або продукт", topicPlaceholder: "Наприклад: нова колекція десертів", details: "Опис завдання", optional: "необов’язково", detailsPlaceholder: "Розкажіть про аудиторію, головну перевагу або бажаний заклик до дії", generate: "Згенерувати контент", generating: "AI пише текст…", result: "Ваш результат", resultHint: "Готовий текст з’явиться тут", empty: "Тут буде ваш текст", emptyHint: "Заповніть форму ліворуч і натисніть «Згенерувати контент».", copy: "Копіювати текст", copied: "✓ Текст скопійовано", ideas: "Не знаєте, з чого почати?", validation: "Введіть тему або назву продукту — без неї AI не зможе почати.", fallback: "Щось пішло не так. Спробуйте ще раз.", types: [["Instagram допис", "публікація із закликом"], ["Instagram Stories", "серія залучаючих екранів"], ["Telegram допис", "живий текст для каналу"], ["Рекламне оголошення", "три короткі варіанти"], ["Опис товару", "переваги та характеристики"], ["Ідея контент-плану", "план публікацій на 7 днів"]], examples: ["нова колекція десертів", "акція на кросівки"] },
  en: { lang: "en", title: "ContentAI Generator — content that works", ai: "Your AI copywriter", badge: "Social and business copy in minutes", hero: ["You have the idea.", "Words will follow."], intro: "Choose a format, tell us about your task, and ContentAI will prepare copy people want to read.", create: "Create content", steps: "Choose a format and describe your task", language: "Result language", format: "Choose a format", topic: "Topic or product", topicPlaceholder: "For example: a new dessert collection", details: "Task description", optional: "optional", detailsPlaceholder: "Tell us about the audience, key benefit, or desired call to action", generate: "Generate content", generating: "AI is writing…", result: "Your result", resultHint: "Your finished copy will appear here", empty: "Your copy will appear here", emptyHint: "Complete the form on the left and click “Generate content”.", copy: "Copy text", copied: "✓ Text copied", ideas: "Not sure where to start?", validation: "Enter a topic or product name so AI can get started.", fallback: "Something went wrong. Please try again.", types: [["Instagram post", "a post with a call to action"], ["Instagram Stories", "an engaging story sequence"], ["Telegram post", "lively copy for your channel"], ["Ad copy", "three short variations"], ["Product description", "benefits and features"], ["Content plan idea", "a seven-day publishing plan"]], examples: ["a new dessert collection", "a sneaker sale"] },
} as const;
  ...
  async function generate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/generate", {
        ...
        body: JSON.stringify({ topic, details, contentType, language }),
      });
